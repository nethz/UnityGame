#pragma strict
class BallBind  extends MonoBehaviour{
	...
	public var _sound:BallBind.Sound = null;
	function Start () {
	...
	function setType(type:Geek.MagicType){
		var btb:BallTypeBind = this.GetComponent(BallTypeBind) as BallTypeBind;
		btb.setType(type);
	}
	...
	function spallTask(type:Geek.MagicType){
		var spall:SpallTask = new SpallTask(this._spallPrototype);
	...
	function spall()
	{
		var bbb:BallBurstBind = this.GetComponentInChildren(BallBurstBind);
		bbb.burst();
	...
	function setFlash(flash:boolean){
		...
		var bfb:BallFlashBind = this.GetComponentInChildren(BallFlashBind);
		bfb.setFlash(flash);
	}
