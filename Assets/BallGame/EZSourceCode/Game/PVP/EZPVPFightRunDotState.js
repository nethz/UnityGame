...
class EZPVPFightRunDotState extends StateWithEventMap{
	private var isOver_:boolean;
	
	
	//private var techniqueHandler_:EZTechniqueHandler;
	function EZPVPFightRunDotState(){
	...
	function doDot(tl:TaskList, dot:EZDot){
		...
		tl.push(task);
		EZTechniqueHandler.PVPDoDot(tl, context.data, context.from, context.to);
		
		
	}
	function start(){
		
		Debug.Log("EZPVPFightRunDotState");
		isOver_ = false;
