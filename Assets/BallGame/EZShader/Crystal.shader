...
Shader "EZ/Crystal" {
    Properties {
        _BallTex ("Atlas Texture", 2D) = "white" {}
        _LightTex ("Atlas Texture", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
			...
			fixed4 frag ( v2f _in ) : COLOR {
				fixed4 ballColor = tex2D ( _BallTex, _in.texcoord ) * _in.color;
				fixed4 lightColor = tex2D ( _LightTex, _in.texcoord ) * _in.color;
          
            	fixed4 outClor = ballColor + lightColor;
